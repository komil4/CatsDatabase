...
-- Задание 2
INSERT INTO cats_stat (tail_length_mean, whiskers_length_mean, tail_length_median,
    whiskers_length_median, tail_length_mode, whiskers_length_mode)
WITH
    avgCats AS (
    SELECT AVG(tail_length) AS tail_length_mean, AVG(whiskers_length) AS whiskers_length_mean
    FROM cats),
    medianCats AS (
    SELECT TOP(1)
        PERCENTILE_CONT(0.5)
        WITHIN GROUP (ORDER BY tail_length)
        OVER() AS tail_length_median,
        TOP(1) PERCENTILE_CONT(0.5)
        WITHIN GROUP (ORDER BY whiskers_length)
        OVER() AS whiskers_length_median,
    FROM cats),
    tailModeCats AS (
        SELECT TOP(1) WITH TIES tail_length AS tail_length_mode
        FROM cats
        GROUP BY tail_length
        ORDER BY COUNT(*) DESC),
    whiskersModeCats AS (
        SELECT TOP(1) WITH TIES whiskers_length AS whiskers_length_mode
        FROM cats
        GROUP BY whiskers_length
        ORDER BY COUNT(*) DESC)
)
SELECT avgCats.tail_length_mean AS tail_length_mean, avgCats.whiskers_length_mean AS whiskers_length_mean,
    medianCats.tail_length_median AS tail_length_median, medianCats.whiskers_length_median AS whiskers_length_median,
    tailModeCats.tail_length_mode AS tail_length_mode, whiskersModeCats.whiskers_length_mode AS whiskers_length_mode
FROM avgCats, medianCats, tailModeCats, whiskersModeCats;
